...
npm run process:ui-assets # public/assets/particles/*.JPG → 去背 PNG(粒子素材)
...
| 元件 | class | alpha | blur | z-index |
|---|---|---|---|---|
| HUD（top） | `.hud` | 0.35（米白） | 20 / saturate 140% | 40 |
| BottomBar | `.hud-bottom` | 0.35（米白） | 20 / saturate 140% | **60** |
| Modal 後幕 | `.modal-backdrop` | 0.25（黑） | 8 | 50 |
| Modal 抽屜 | `.glass-popup` | 0.75（米白） | 24 / saturate 150% | 50 |
...
### `.glass-popup` 定位約定(踩過的雷)

**不**用 `height: calc(100vh - ...)` — iOS PWA 算的 vh 會差 5-10px,
彈窗底部和 BottomBar 對不齊。改用 `top: 140px; bottom: 0;
padding-bottom: calc(var(--bottom-bar-height) + env(safe-area-inset-bottom))`,
讓 popup 直接黏到 viewport 底,內容區用 padding-bottom 預留
BottomBar 高度,任何裝置都對齊。

`.hud-bottom` z-index 60 **大於** `.modal-backdrop` z-50,
所以開彈窗時 BottomBar 還能浮在後幕上面,維持 5 顆按鈕始終可見的視覺整體感。

### HUD 主題色 CSS 變數

`<html data-theme="default|jade|purple|red">` 上掛 4 顆變數:
`--hud-bg` / `--hud-border` / `--popup-bg` / `--popup-border` / `--popup-header-bg`。
切換主題色時只更新 `data-theme`,變數即時生效,不用 reload。
...
| 位置 | icon | 開的 modal | 內容 |
|---|---|---|---|
| 1 | 🎮 game.png | `GameModal` | 4 tabs:任務 / 成就 / 圖鑑 / 修為(原本散在紀錄頁的養成內容) |
| 2 | 👥 friends.png | `FriendsModal` | 「🚧 即將推出」placeholder(階段 5 預留) |
| 3 | 🛒 trade.png | `TradeModal` | 3 顆大按鈕:🥚買入 / 🍖餵食 / 📦售出,各自 trigger BuyModal/FeedModal/SellModal |
| 4 | 📜 records.png | `RecordsModal` | 3 tabs:圖表 / 對比 / 交易紀錄(只剩查資料,養成內容已搬 GameModal) |
| 5 | ⚙️ settings.png | `SettingsModal` | 玩家名 / 手續費 / 音效 / HUD 主題 / 家園背景 / 雲端同步 / 刪帳號 |

### 紅點 badge

任務「completed && !claimed」count > 0 → 第 1 顆 game 按鈕右上角紅點。
不再放在記錄按鈕(R 改版前是)。

### 子頁設計約定:state-based,**不要**用 nested Modal

`SettingsModal` 內的 HUD 主題色 / 家園背景兩個入口,點下去**不**開新 Modal,而是 `setSubView('hudTheme' | 'background')` 切換,sub-view 元件(`HudThemeSection` / `BackgroundSection`)純內容,共用 SettingsModal 的 `.glass-popup-content` 渲染。

`Bestiary` 內的 50 隻詳細頁同理,點卡片 `setSelectedSpeciesId(id)`,
`BestiaryPetDetail` 取代列表渲染在同一個 `.glass-popup-content` 內,**不**包 Modal。

**為什麼**:見下方雷區「iOS Safari backdrop-filter 創 containing block」。
nested Modal 的 `position: fixed` 會被 outer modal 的 backdrop-filter 框住,
導致子頁 popup 鎖進 outer popup 範圍,底部沒貼齊 BottomBar(PR #75/#76/#85 連環踩雷)。
...
### 消耗來源
階段 4A / 4B / 4C 全部實作完畢,共 8 個 reason 代碼。階段 4A 四項在 `PetInfoModal` 底部(每隻神獸獨立花費);階段 4B 兩項在 `SettingsModal` 子頁(全域設定);階段 4C 兩項在 `BestiaryPetDetail`(圖鑑詳細頁,state-based view 不是 Modal)。
| 按鈕位置 | reason | 金額 | 效果 |
|---|---|---|---|
...
| SettingsModal → HudThemeSection | `theme` | 200 | 解鎖一次永久持有;切換時 `<html data-theme>` 即時切 CSS 變數 |
| SettingsModal → BackgroundSection | `background` | 500 | 解鎖一次永久持有;切換時 `WorldScene.setBackgroundId` 動態載入 texture |
| BestiaryPetDetail:永恆紀念 | `eternal` | 2000 | `pet.isEternal=true`,圖鑑卡金邊 + ✨ 角標 + finalEffect 永久動態 |
| BestiaryPetDetail:解鎖傳說 | `unlock_story` | 100 | `creatureUnlocks` 表 append,per-creatureId 永久解鎖(賣光重買仍解鎖) |
...
- 階段 R 重構後:修為 tab(`CultivationTab`)搬進 `GameModal` 4 tabs 之一,不再在 `RecordsModal`
...
11 個 emit 點:
  ...
  - `RecordsModal` overview tab → `view_chart`(R 改版後只剩圖表/對比/交易紀錄 3 tabs)
  - `GameModal` bestiary tab → `view_codex`(R 改版後圖鑑搬到 GameModal)
  - `RecordsModal` transactions tab → `view_records`
...
- **iOS Safari `backdrop-filter` 創 containing block(最最痛的雷)**:任何 element 套 `backdrop-filter: blur(...)` 在 iOS Safari 上會變成 `position: fixed` 子孫的 containing block(spec 標的 bug,Chrome 不會這樣)。`.glass-popup` 跟 `.modal-backdrop` 都有 backdrop-filter,所以**包在 Modal 內的 nested Modal**(舊版 BestiaryPetModal / HudThemeModal / BackgroundModal)的 fixed 定位會解析成 outer popup 的 box 邊界,而非 viewport,結果子頁鎖進 outer popup 範圍、底部沒貼齊 BottomBar。**正解**:子頁不用 nested Modal,改 state-based view — outer Modal 內 `subView` state,sub-view 元件(`BestiaryPetDetail` / `HudThemeSection` / `BackgroundSection`)純內容,共用 outer `.glass-popup-content` 渲染。PR #75/#76/#85 連環踩到三次才搞清楚規律。
- **`.glass-popup` 不可用 `height: calc(100vh - X)` 算高**:iOS PWA 算的 vh 跟桌機 / Chrome 不一樣,popup 底部會差 5-10px,跟 BottomBar 對不齊。改用 `top: 140px; bottom: 0; padding-bottom: calc(var(--bottom-bar-height) + env(safe-area-inset-bottom))`,直接黏到 viewport 底,內容區 padding-bottom 預留 BottomBar 高度,任何裝置都對齊(PR #85 修)。
- **BottomBar z-index 必須 > Modal backdrop**:modal-backdrop 是 z-50,Modal popup 也 z-50。BottomBar 預設 z-30 開彈窗會被後幕 + popup 蓋住,5 顆按鈕消失。改 `.hud-bottom { z-index: 60 }`,讓 BottomBar 永遠浮在最上層,popup 用 padding-bottom 預留空間不會擠到按鈕。
- **Dexie `.where('unindexedField')` 直接 throw SchemaError**:Bestiary 用 `db.pets.where('speciesId').count()` 但 `speciesId` 不在 indexes 內 → throw → `useLiveQuery` rethrow 在 render 階段 → React 整棵子樹 unmount → 白屏。**正解**:換 `.toArray()` + memory filter(`pets.filter(p => p.speciesId === id).length`),加 `ErrorBoundary` 包敏感區段保底(PR #75 修)。
- **`useLiveQuery` 在 render 階段會 rethrow 上游 throw**:liveQuery callback 內若 throw(例如上面的 SchemaError),React render 直接爆,不是優雅 fallback。所以**敏感 query 要包 try/catch 或先檢查 schema**,否則用 `ErrorBoundary` 包元件,白屏退化成「⚠️ 載入失敗」訊息。
- **TypeScript `useLiveQuery(() => Promise.resolve([]))` 推成 `Promise<never[]>`**:會把整個 query return type 收窄成 `never[]`。**正解**:寫 generic `useLiveQuery<Pet[]>(async () => { if (!id) return []; ... })` 或第一行 `return [] as Pet[];`。
- **PetInfoModal sub-action modals 仍是 nested Modal**:`RenameModal` / `BoostRealmModal` / `TemperRingModal` / `ColorVariantModal` 都 nested 在 `PetInfoModal` 內,理論上踩 iOS containing block 雷,但目前 user 沒 report,先放著不動。日後若報 bug,參考 SettingsModal 改 state-based subView 一招套用。
- **`process-button-icons.mjs` 早期版本 white-flood 失敗**:第一版用 `minRgb < 230` 判殘留 bg,只能處理白底 icon,records.png 換成「深色圓盤金邊」風後白底偵測整片 false → 圖原樣輸出沒去背。**正解**:跟 ring 處理一樣改「4 角 alpha>50 像素 RGB 取平均當 seed → delta-based BFS」,適用任何單色背景。
...
- ❌ `HudThemeModal.tsx` / `BackgroundModal.tsx` / `BestiaryPetModal.tsx`(舊版 nested Modal,已改 state-based view → `HudThemeSection` / `BackgroundSection` / `BestiaryPetDetail`,踩 iOS containing block 雷)
- ❌ `RecordsModal` 7 tabs(原本含 任務/成就/圖鑑/修為,階段 R 改版後搬到 `GameModal`,RecordsModal 精簡剩 圖表/對比/交易紀錄 3 tabs)
- ❌ `RecordsModal` 紅點 badge(階段 R 後紅點搬到 BottomBar 第 1 顆 game 按鈕,因任務搬到 GameModal)
- ❌ `.glass-popup { height: calc(100vh - ...) }`(iOS PWA vh 算錯 5-10px,改 `top:140 + bottom:0`,PR #85)
- ❌ BottomBar 預設 z-index 30(會被 modal backdrop z-50 蓋住,改 z-60 永遠浮在最上層)
- ❌ Dexie `.where('unindexedField')`(throw SchemaError → useLiveQuery rethrow → 白屏,改 toArray + memory filter,PR #75)
- ❌ `process-button-icons.mjs` 白底閾值偵測(只能處理白底 icon,改 4 角 seed-based flood-fill 適用任何單色背景)
- ❌ `public/assets/btn/*.JPG` 5 顆舊按鈕圖示(buy/feed/sell/records/settings 原版,已被 MJ 新圓盤風 PNG 取代)
