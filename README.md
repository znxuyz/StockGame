...
買股票養成神獸的同時，玩家累積「修為點數」。階段 1-4 全部完成:階段 1（養成）+ 階段 2（修為）+ 階段 3（簽到任務）+ 階段 4A（基礎消耗：改名 💎50 / 催熟 💎100 / 淬煉 💎500）+ 階段 4B（進階消耗：換配色 💎300 / HUD 主題色 💎200 / 家園背景 💎500）+ 階段 4C（深度消耗：永恆紀念 💎2000 / 圖鑑故事解鎖 💎100）。
...
| 每日簽到 | +10 |
| 連登里程碑 7/14/30/60/100 | +100 / +200 / +500 / +1,000 / +2,000 |
| 完成每日任務 | +20 ~ +30 |
| 完成週任務 | +100 ~ +200 |

修為動畫：HUD 💎 數字 count-up + 綠色 +N 飄字 1.5 秒上飄漸出，amount ≥ 100 加金色光圈。紀錄頁「修為」tab 看完整變動歷史。

## 每日簽到 + 任務系統

建立日常循環：每天打開 App → 領簽到 → 推任務 → 累積修為 → 養神獸。

| 機制 | 規則 |
|---|---|
| **連登簽到** | 每天首次打開 App 自動跳簽到彈窗，領 +10 修為。連登中斷重設為 1 |
| **里程碑** | 連登 7/14/30/60/100 日領取時觸發全螢幕慶祝動畫（黑幕 + 金色光柱 + 中央文字） |
